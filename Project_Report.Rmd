...
2023) and an annual dataset (2006 - 2023). These datasets provide
detailed insights into cinema admissions, audience spending, economic
...
```{r plot_monthly_raw, fig.cap='Overview of the Monthly Dataset', include=FALSE}
...
```{r missing_vs_imputed, fig.show='hold', fig.cap="Original vs. Imputed Dataset"}
...
for (col in columns_to_impute) {
  ...
  plot(ott_cinema_monthly.ts[, col], type = "o", pch = 16, lty = 3, cex = 0.4,
       main = paste("Original", col), xlab = NA, ylab = col)
...
```{r monthly_imputed, include=FALSE}
...
```{r plot_annual_raw, fig.cap='Overview of the Annual Dataset', out.width="70%"}
...
```{r plot_prophet_residuals_arima}
...
checkresiduals(prophet.arima_residuals)
```

```{r prophet_arima_metrics}
model_name <- "Prophet + ARIMA"
aic <- NA
dw <- NA
lj_box <- sprintf("%.3f", checkresiduals(prophet_data$combined_residuals)$p.value)
...
checkresiduals(hw.cinema_admissions)
```
...
future <- make_future_dataframe(prophet_model, periods = 12*3, freq = "month", include_history = TRUE)
...
prres.arima_forecast <- forecast(prres.arima, h = 12*3)
```


```{r prop_ari_forecast}
# Final combined forecast
final_forecast <- tail(prophet_forecast$yhat, 12*3) + as.numeric(prres.arima_forecast$mean)
forecast_dates <- tail(future$ds, 12*3)
```


```{r prophet_arima_CI}
# Prophet forecast intervals (80% CI by default)
prophet_upper <- tail(prophet_forecast$yhat_upper, 12*3)
prophet_lower <- tail(prophet_forecast$yhat_lower, 12*3)

# ARIMA forecast intervals (95% CI)
arima_upper <- prres.arima_forecast$upper[,2]
arima_lower <- prres.arima_forecast$lower[,2]

# Combined intervals
combined_upper <- prophet_upper + arima_upper
combined_lower <- prophet_lower + arima_lower

```


```{r prophet_arima_forecast_plot}
# Combine historical and forecast dates
full_dates <- c(ott_cinema_monthly$ds, forecast_dates)
y_range <- range(c(ott_cinema_monthly$cinema_admissions, final_forecast, 
                   combined_upper, combined_lower), na.rm = TRUE)

# First: Historical plot + fitted values
plot(ott_cinema_monthly$ds, ott_cinema_monthly$cinema_admissions, type = "o", 
     pch = 16, cex = 0.6,
     xlab = "Date", ylab = "Cinema Admissions", 
     main = "Cinema Admissions with Prophet + ARIMA Forecast",
...
# Add confidence interval (shaded band)
polygon(c(as.Date(forecast_dates), rev(as.Date(forecast_dates))),
        c(combined_upper, rev(combined_lower)),
        col = rgb(0, 0, 1, 0.2), border = NA)  # light blue transparent

# Add forecast line
lines(as.Date(forecast_dates), final_forecast, col = "blue", lwd = 2, lty = 2)

# Add legend
legend("topright", 
       legend = c("Prophet (Fitted)", "Prophet + ARIMA (Fitted)", "Prophet + ARIMA Forecast"),
       col = c("orange2", "purple2", "blue"), cex = 0.8,
       lty = c(1, 1, 2), lwd = c(2, 2, 2), pch = c(NA, NA, NA), pt.cex = 0.5, bty = "n")
...
polygon(c(forecast_dates, rev(forecast_dates)),
        ...
        col = rgb(0, 0, 1, 0.2), border = NA)  # semi-transparent 
...
!!! TODO: SINCE THERE IS A SCALE DIFFERENCE IN THE DATA, THE PLOT DOESNT EXPLAIN MUCH. CAN I WORK WITH NORMALIZED DATA WHEN MODELING UCRCD? Will it be academically acceptable? Let's try:
